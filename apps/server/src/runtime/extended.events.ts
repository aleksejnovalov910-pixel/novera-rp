...
export function registerExtendedEvents(d:Deps):void{
  ...
  mp.events.add(PhoneEvents.addContact,async(p:PlayerMp,targetRaw:string,alias:string)=>{const c=getPlayerContext(p);if(!c||!d.guard.allow(p,'phone.contact',5,2000))return;try{const ok=await d.phone.addContact(c.characterId,BigInt(targetRaw),String(alias));send(p,PhoneEvents.state,{ok,contacts:ok?await d.phone.contacts(c.characterId):undefined})}catch{send(p,PhoneEvents.state,{ok:false})}});
  mp.events.add(PhoneEvents.sendMessage,async(p:PlayerMp,targetRaw:string,body:string)=>{const c=getPlayerContext(p);if(!c||!d.guard.allow(p,'phone.message',4,2000))return;try{const target=BigInt(targetRaw);const id=await d.phone.send(c.characterId,target,String(body));send(p,PhoneEvents.state,{ok:id!=null,messageId:id?.toString(),targetId:target.toString(),messages:id?await d.phone.conversation(c.characterId,target):undefined})}catch{send(p,PhoneEvents.state,{ok:false})}});
  mp.events.add(PhoneEvents.conversation,async(p:PlayerMp,targetRaw:string)=>{const c=getPlayerContext(p);if(!c||!d.guard.allow(p,'phone.conversation',8,1000))return;try{const target=BigInt(targetRaw);send(p,PhoneEvents.conversation,{ok:true,targetId:target.toString(),messages:await d.phone.conversation(c.characterId,target)})}catch{send(p,PhoneEvents.conversation,{ok:false})}});
