...
await writeFile(resolve(runtime,'packages/novera/package.json'),JSON.stringify({name:'novera-runtime',version:'0.10.0',private:true,main:'index.js'},null,2));
await writeFile(resolve(runtime,'client_packages/index.js'),"require('./novera/client/index.js');\n");
await writeFile(resolve(runtime,'DEPLOY.txt'),'NOVERA RP GTA5HOST runtime v0.10.0. Apply SQL migrations in order. Configure the database connection before launch. Upload the contents of this directory to the RAGE:MP server root.\n');
