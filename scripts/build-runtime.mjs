...
// GTA5HOST currently runs a legacy Node runtime that does not resolve the node: protocol.
// Target Node 14 syntax and then normalize every bundled node:* require/import to its classic builtin name.
...
await writeFile(resolve(runtime,'packages/novera/package.json'),JSON.stringify({name:'novera-runtime',version:'0.15.2-alpha',private:true,main:'index.js'},null,2));
...
await writeFile(resolve(runtime,'START_HERE.txt'),[
  'NOVERA RP v0.15.2 Alpha — GTA5HOST legacy Node + auth UI hotfix',
  '',
  'This build fixes GTA5HOST legacy Node compatibility (node:* builtin imports), the auth server-response path and RAGE CEF black compositing rectangles.',
  ...
  'Keep your existing working novera.config.json with the real DATABASE_URL. The distributed file contains placeholders by design.',
  'After restart the console must NOT contain: Cannot find module node:fs (or any other node:* builtin).',
  '',
