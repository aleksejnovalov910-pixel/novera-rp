...
await build({
  ...
  outfile: resolve(runtime, 'packages/novera/index.js'),
  bundle: true,
  platform: 'node',
  format: 'cjs',
  target: 'node18',
...
await writeFile(resolve(runtime, 'packages/novera/package.json'), JSON.stringify({ name: 'novera-runtime', version: '0.7.0', private: true, main: 'index.js' }, null, 2));
await writeFile(resolve(runtime, 'client_packages/index.js'), "require('./novera/client/index.js');\n");
await writeFile(resolve(runtime, 'DEPLOY.txt'), 'NOVERA RP GTA5HOST runtime. Apply SQL migrations in order. Configure the database connection before launch. Upload the contents of this directory to the RAGE:MP server root.\n');
