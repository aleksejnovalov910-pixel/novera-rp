...
await writeFile(resolve(runtime,'packages/novera/package.json'),JSON.stringify({name:'novera-runtime',version:'0.14.1-alpha',private:true,main:'index.js'},null,2));
...
await writeFile(resolve(runtime,'START_HERE.txt'),[
  'NOVERA RP v0.14.1 Alpha — GTA5HOST',
  '',
  '1. Open novera.config.json and replace CHANGE_ME_USER, CHANGE_ME_PASSWORD and CHANGE_ME_DATABASE with the MySQL data from GTA5HOST.',
  '2. Keep MySQL host as 127.0.0.1 when the game server and MySQL are on the same GTA5HOST service.',
  '3. Import migrations/ALL_MIGRATIONS.sql once into the empty NOVERA database using phpMyAdmin.',
  '4. Upload conf.json, novera.config.json, packages/, client_packages/ and migrations/ to the RAGE:MP server root.',
  '5. Do not upload GitHub source folders, pnpm files or development node_modules.',
  '6. Start the server and copy the first console output/error into ChatGPT for runtime debugging.',
  '',
